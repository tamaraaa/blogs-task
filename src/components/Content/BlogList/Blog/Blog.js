...
const Blog = ({ blog, deletePost, categoryId, setCurrnetPost, setShowModal, setShowCategoryModal }) => {
	...
	const placeholder = Array(3).fill(
		...
		<div className="blog">
			<div className="blog__header">
				...
				<span className="blog__header__buttons">
					<button className="blog__header__buttons__edit" onClick={handleEdit}>
						Edit
					</button>
					<button
						className="blog__header__buttons__delete"
						onClick={() => deletePost(blog.id, categoryId)}>
						Delete
					</button>
				</span>
			</div>
			...
			<div className="blog__images">
				{placeholder.map((item, index) => (
					<img className="blog__images__image" key={index} src={item} />
				))}
			</div>
		</div>
	);
